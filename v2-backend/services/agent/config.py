...
class AgentConfig:
    ...
    loop_threshold: int = 3           # Warn after this many identical calls
    loop_force_stop: int = 5          # Hard stop: strip tool_calls, force text output
    loop_window: int = 20
    parallel_tool_workers: int = 4
    compact_threshold: int = 80000  # Auto-compact when prompt tokens exceed this
...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "6. 当你有足够信息时，给出最终文字回答\n\n"
    "## 元认知规则（必须遵守）\n"
    "1. **任务规划**: 收到多步骤任务时，必须先调用 think 工具完成：重述目标、识别风险、选择策略、定义完成标准\n"
    "2. **进度检查**: 每执行 3 次工具调用后，必须调用 think 工具做进度检查：原始目标是什么、已完成什么、是否在正轨、是否需要调整\n"
    "3. **结果评估**: 工具返回结果后，评估结果质量：是否完整、是否回答了问题、置信度如何\n"
    "4. **不确定性**: 遇到不确定的决策时，明确表达不确定性而非猜测\n\n"
    "重要规则:\n"
...
## 元认知规则
- 开始处理前，先用 think 工具规划：重述目标、识别风险、选择策略
- 每执行 3 次工具调用后，用 think 评估进度：已完成什么、是否在正轨、是否需要调整
- 工具返回错误时，先用 think 分析原因再重试
- 不确定时明确告知用户，不要猜测
...
def create_provider(llm_config: LLMConfig):
    ...
    if provider_name == "gemini":
        ...
        return KimiProvider(llm_config)
    else:
        raise ValueError(f"Unknown LLM provider: {provider_name}. Supported: gemini, openai, deepseek, kimi")
