...
def fill_template(
    ...
    # ── Step 0d: Capture summary-zone merge pattern from template ──
    # The original template has merges inside the summary zone (e.g.
    # `I33:J33` for a "Subtotal" label). After we shift rows, openpyxl does
    # NOT update merge ranges, so we capture them by relative offset now and
    # re-apply at the new summary position in Step 7b.
    #
    # Without this step, every row-resized template loses its summary merges
    # entirely — verify_template_contract then complains about missing merges
    # at the new summary position, blocking inquiry generation.
    summary_merge_offsets: list[tuple[int, int, int, int]] = []
    for mr in ws.merged_cells.ranges:
        if (
            mr.min_row >= summ_zone["start"]
            and mr.max_row <= summ_zone["end"]
        ):
            summary_merge_offsets.append((
                mr.min_row - summ_zone["start"],
                mr.max_row - summ_zone["start"],
                mr.min_col,
                mr.max_col,
            ))
    ...
    # ── Step 7b: Re-apply summary merges at new positions ────────
    # The original template had merges inside the summary zone (captured in
    # Step 0d). Row insertion did NOT shift them, and the stale-cleanup in
    # Step 1b removed any leftovers. Recreate them now at the new summary
    # row positions so verify_template_contract sees the expected merge
    # structure and downstream consumers (Excel viewers, formula engines)
    # render the summary section identically to the template.
    if summary_merge_offsets:
        for start_offset, end_offset, start_col, end_col in summary_merge_offsets:
            new_min_row = new_summ_start + start_offset
            new_max_row = new_summ_start + end_offset
            start_letter = get_column_letter(start_col)
            end_letter = get_column_letter(end_col)
            merge_ref = f"{start_letter}{new_min_row}:{end_letter}{new_max_row}"
            try:
                ws.merge_cells(merge_ref)
            except Exception as exc:
                logger.warning("Failed to re-apply summary merge %s: %s", merge_ref, exc)
        logger.info(
            "Re-applied %d summary merge range(s) at new positions",
            len(summary_merge_offsets),
        )
...
def verify_output(
    ...
    for i, product in enumerate(products):
        ...
        for col_letter, field_name in col_map.items():
            ...
            # Treat None and empty string as the same "blank cell" state.
            # openpyxl normalizes "" to None when serializing the workbook,
            # so any field that resolves to "" (typically because the source
            # product dict doesn't carry that field at all — e.g. order_status)
            # will read back as None. Failing those would produce N false
            # negatives per row and block legitimate inquiry generation.
            expected_is_blank = expected is None or expected == ""
            actual_is_blank = actual is None or actual == ""
            if expected_is_blank and actual_is_blank:
                continue
            if expected_is_blank:
                # expected is blank but actual has content — that's stale data
                errors.append(f"Row {row} {col_letter} ({field_name}): expected blank, got '{actual}'")
                continue
            if actual_is_blank:
                errors.append(f"Row {row} {col_letter} ({field_name}): expected '{expected}', got blank")
                continue
    ...
    # ── 7. Template contract structural checks ──────────────────
    template_contract = zone_config.get("template_contract")
    if template_contract:
        contract_errors, contract_checks = verify_template_contract(
            ws=ws,
            template_contract=template_contract,
            product_start_row=prod_start,
